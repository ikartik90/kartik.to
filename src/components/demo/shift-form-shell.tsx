...
// Teeth on x-percentages at a fixed pixel depth; peaks on the corners keep the sides straight.
...
// Only the bottom row of `tornEdgesClip`, same phase, for a one-tooth band.
...
// Row heights for demos that size the card; must match the styles below.
...
// The block padding holds the 20px teeth on top of the 16px inset.
const formStyle = css({
...
// Empty, it takes no height; a fill owns its padding so it travels when animated.
...
const footerWrapStyle = css({
  "&::before": {
    ...
    backgroundSize: "100% 100%",
    backgroundRepeat: "no-repeat",
    // The exported path points down; flipped so the tear points up at the form.
    transform: "scaleY(-1)",
  },
});
...
export interface ShiftFormShellProps {
  /** The live form surface. */
  children: ReactNode;
  /** Wireframe filling the footer (v1's counterweight); ignored when `cropped`. */
  footerFill?: ReactNode;
  /** End at the tear with no action bar; the body is clipped. */
  cropped?: boolean;
}
